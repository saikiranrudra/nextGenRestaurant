      ...
      <List>
        {props.recivedOrders.map((item, index) => (
          <ListItem disabled key={index}>
            <ListItemText>
              <span style={{ fontFamily: "Product-Sans" }}>{item.name}</span>
            </ListItemText>
            <ListItemText style={{ textAlign: "end" }}>
              <span style={{ fontFamily: "Product-Sans" }}>
                Normal: {item.normalCount ? item.normalCount : 0}
              </span>
              <span style={{ marginLeft: ".8rem", fontFamily: "Product-Sans" }}>
                Jain: {item.jainCount ? item.jainCount : 0}
              </span>
            </ListItemText>
          </ListItem>
        ))}
